 ...
 *
 * That last block is NOT on this stick. Checked 2026-09-14 against a DTS-WL-G3
 * using the layout from the ha-hoymiles-dtupro project: the microinverter block
 * starts at 0x1000 with a stride of 40 and carries the inverter SERIAL in its
 * first words, which makes it trivially falsifiable. FC04 over 0x1000-0x100D
 * came back all zeros; FC03 returned configuration values (1 1 320 200 600 540
 * 564 20 584 3000). No serial anywhere, and the attached HiOne-16T's own
 * serial appears in neither. That block belongs to a DTU-Pro sitting in
 * front of HMS/HM microinverters; this stick does not implement it.
 *
 * Consequence: the zeros at 0x1010 are unmapped space, NOT "no DC production".
 * Do not build a DC PV capability on them - there is no known register for the
 * HiOne's own DC strings, so such a tile could not be verified by anyone.
 *
...
const BATTERY_REGISTERS = {
  ...
  // The SMART PORT total, not a DC string — these are the three PHASE registers
  // (a DC array has no phases), so whatever hangs on that port is in the sum.
  // There is no single total register: 0x0056 looked like one but stayed at 228
  // while production fell, so it is a different quantity.
  ...
  // That only proves it tracks solar, NOT that solar is all it carries. On a
  // site with an AC-coupled inverter AND two batteries on the same smart port,
  // this read 2452 W while the solar inverter itself reported 583 W and the two
  // batteries were discharging 1994 W (2026-09-14) — and it FELL to 1035 W as
  // those batteries stopped, while the sun rose to 1209 W. So do not use it as
  // an "is the sun shining" signal; read the PV inverter's own meter for that.
...
class HoymilesModbus {
  ...
  async setReserveSoc(percent, mode) {
    ...
    // and the card would show a floor the battery is not using. _writePercent
    // skips a register that already holds the value, so a shadow that is
    // already zero costs a read and no EEPROM cycle.
  ...
  async _writePercent(def, percent) {
    ...
    // Read before writing, and skip the write when the register already holds
    // this value. These settings are written by Flows on a timer — the price
    // ladder every five minutes, the Sessy guards every minute — and almost
    // every one of those asks for the value that is already there. Each write
    // is an EEPROM cycle on the inverter, so the read is the cheap half of the
    // trade. A read that is not trustworthy simply falls through to the write.
    const [current] = await this.readRegisters(def.addr, 1, { input: false });
    if (current === raw) {
      this.log(`[Modbus] ${this._hex(def.addr)} already ${raw} (${pct}%) - no write`);
      return pct;
    }
    await this._gap();
