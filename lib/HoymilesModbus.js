 ...
 *
 * The battery mode itself stays cloud-only — no register anywhere reflects it,
 * checked across a real mode round-trip.
 *
...
const RESERVE_SOC_BY_MODE = {
  1: 0x10CA, // Self-Consumption
  5: 0x10CF, // Force Charge
};
...
class HoymilesModbus {
  ...
  /**
   * Write the reserve SOC of the given (active) battery mode.
   */
  async setReserveSoc(percent, activeMode) {
    const addr = RESERVE_SOC_BY_MODE[Number(activeMode)];
    if (addr === undefined) {
      throw new Error(`No local reserve-SOC register known for battery mode ${activeMode}`);
    }
    return this._writePercent({ addr, factor: 1, min: 0, max: 100 }, percent);
  }
