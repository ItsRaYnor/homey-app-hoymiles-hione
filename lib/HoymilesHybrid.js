...
class HoymilesHybrid {
  constructor({ gatewayIp, localPort, email, password, stationId, log, error, baseUrl }) {
    this._email     = email;
  ...
  /**
   * Output power limit — local-only (LAN protocol, hoymiles-wifi style).
   */
  async setPowerLimit(limitPercent) {
    if (!this._local) throw new Error('Power limit requires a local gateway connection (set the gateway IP)');
    await this._local.setPowerLimit(limitPercent);
    return 'local';
  }

  /**
   * Inverter on/off by serial — local-only (LAN protocol).
   */
  async setInverterState(serial, on) {
    if (!this._local) throw new Error('Inverter on/off requires a local gateway connection (set the gateway IP)');
