...
class HoymilesHybrid {
  ...
  async getGatewayInfo() {
    // Prefer the local native protocol when available — it works offline and
    // doesn't spend a cloud token. A Modbus-only stick has no native equivalent,
    // so skip 10081 entirely and go straight to the cloud fallback below.
    if (this._local && this._protocol !== 'modbus') {
      try {
        const info = await this._local.getGatewayInfo();
        if (info && info.dtuSn) return info;
      } catch (err) {
        this.log('[Hybrid] Local gateway info failed, trying cloud: ' + err.message);
      }
    }

    // Cloud fallback: the device tree carries the same DTU identity, so
    // cloud-only and Modbus setups still get the gateway-info fields filled.
    if (!this._cloudAvailable()) return null;
    await this._cloud.ensureToken(this._email, this._password);
    return this._cloud.getDevices(this._stationId);
  }
