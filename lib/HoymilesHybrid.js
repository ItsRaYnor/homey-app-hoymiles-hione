...
class HoymilesHybrid {
  ...
  async getBatterySettings() {
    ...
    // The cloud's mode lags a local switch by minutes. While the stick is
    // reachable it reports the mode itself, so a cloud value here would only
    // overwrite a fresh reading with an old one.
    if (settings && settings.mode !== undefined && !this._modbusActive()) {
      this.setKnownMode(settings.mode);
    }
  ...
  /** The mode last read from the stick or last written, or null. */
  getKnownMode() {
    return this._lastKnownMode === undefined ? null : this._lastKnownMode;
  }
