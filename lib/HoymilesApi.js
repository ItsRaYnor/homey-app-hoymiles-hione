...
class HoymilesApi {
  ...
  constructor({ log, error, baseUrl }) {
    ...
    this._bmsDir  = 1;    // last known battery direction (+1 charge / −1 discharge)
    this._lastBms = null; // previous poll's { inEq, outEq, soc } for direction detection
  }
  ...
  async getRealData(stationId) {
    ...
    // values that don't map cleanly onto modes 1–8 (e.g. future AI-mode
    // codes) — the settings read remains the authoritative fallback.
    const wm = Number(reflux.work_mode);
    const workMode = (Number.isFinite(wm) && wm % 1000 === 0 && wm >= 1000 && wm <= 8000)
      ? String(wm / 1000)
      : null;
    ...
    let dir = this._bmsDir || 1;
    if (flowDir !== 0) dir = flowDir;
    else if (workMode === '5') dir = 1;
    else if (workMode === '6') dir = -1;
    else if (this._lastBms) {
      const dIn  = (inEq  !== null && this._lastBms.inEq  !== null) ? inEq  - this._lastBms.inEq  : 0;
      const dOut = (outEq !== null && this._lastBms.outEq !== null) ? outEq - this._lastBms.outEq : 0;
      if (dIn > dOut)      dir = 1;
      else if (dOut > dIn) dir = -1;
      else if (soc !== null && this._lastBms.soc !== null && soc !== this._lastBms.soc) {
        dir = soc > this._lastBms.soc ? 1 : -1;
      }
    }
    this._bmsDir  = dir;
    this._lastBms = { inEq, outEq, soc };
