...
class HoymilesApi {
  ...
  async getRealData(stationId) {
    ...
    return {
      ...
      // Cloud bms_power is positive while DISCHARGING; Homey's home-battery
      // convention is positive = charging, negative = discharging. Negate it so
      // measure_power, the charge/discharge status and Homey Energy all match.
  ...
  /**
   * Fetch the station's device tree and return the gateway (DTU) identity.
   * The top-level node is the DTU stick (e.g. DTS-WL-G3); its children are the
   * inverter(s) and any HiBox. Used as a cloud fallback for the gateway-info
   * settings when the local native protocol (TCP 10081) is unreachable.
   *
   * Returns { dtuSn, softwareVer, deviceVer, model } or null when unavailable.
   * Keys match HoymilesLocal.getGatewayInfo() so the device layer can consume
  ...
  async getDevices(stationId) {
    ...
    const dtu = Array.isArray(response?.data) ? response.data[0] : null;
    if (!dtu) return null;

    return {
      // The cloud reports the serial as a plain decimal string (matches the
      // printed label), so use it verbatim — no hex conversion needed.
